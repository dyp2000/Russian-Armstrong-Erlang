...
Например:
    ...
    -else.
    -define(TRACE(X), void).
    -endif.MISCELLANEOUS SHOR T TOPICS 110
    start() -> loop(5).
    loop(0) ->
      void;
    loop(N) ->
...
Форматирующие коды представлены символом (\~). \~p является аббревиатурой
...
1.    *Обычный синтаксис*: Здесь целые числа записаны так, как вы ожидаете.
Например, 12, 12375 и -23427 являются целыми числами.

2.    *Целое по основанию К*: Целые числа по основанию, отличному от десяти
...
3.    *$ синтаксис*: Синтаксис $С представляет целочисленный код для
...
вычисляются только если это необходимо.
...
упомянули о некоторых небольших разделах, но мы вернемся к ним, когда
