...
В этой главе мы введём понятие библиотек и примитивов Эрланга, которые мы будем использовать для написания распределённых программ. *Распределённые программы* — это программы, которые созданы для работы в сети компьютеров и могут координировать свои действия только через передачу сообщений.
...
<dl>
  ...
  <dd>Мы можем ускорить работу наших программ, организовав работу разных частей программы параллельно на разных машинах.</dd>
  <dt>Надёжность</dt>
  <dd>Мы можем сделать системы устойчивыми к сбоям, спроектировав их для работы на нескольких машинах. Если одна машина выходит из строя, то мы можем продолжить работу на другой машине.</dd>
  <dt>Масштабируемость</dt>
  <dd>С ростом приложения мы рано или поздно исчерпаем возможности даже самой мощной машины. После этого нам придётся добавлять новые машины для наращивания вычислительной мощности. Добавление новой машины должно быть простой операцией, которая не требует больших изменений в архитектуре приложения.</dd>
  <dt>Распределённо-ориентированные приложения</dt>
  <dd>Многие приложения являются распределёнными по своей сути. Если мы пишем многопользовательскую игру или чат, разные пользователи будут разбросаны по всему земному шару. Если в каком-то географическом месте у нас будет много пользователей, то нам захочется разместить вычислительные ресурсы рядом с ними.</dd>
  <dt>Интерес</dt>
  <dd>Большинство интересных программ, которые я пишу — распределённые. Многие из них включают взаимодействие людей и машин по всему миру.</dd>
</dl>

В этой главе мы поговорим о двух основных моделях распределённости:

 * *Распределённый Эрланг*: обеспечивает метод для программирования приложений, которые работают на наборе сильно связанных компьютеров (Например машины, находящиеся в одной локальной сети, занимающейся решением конкретной проблемы.). Распределённые Эрланг программы пишутся так, чтобы работать на *узлах* Эрланга. Мы можем порождать процессы на любом узле и все примитивы передачи сообщений и обработки ошибок, о которых мы говорили в предыдущих главах, работают как для случая одиночного узла, так и на нескольких.<br>
Распределённые Эрланг-приложения работают в *доверенной* среде — т.к. любой узел может выполнить любую операцию на любом другом узле Эрланга, а это подразумевает высокую степень доверия. Типично распределённые Эрланг-приложения работают объединённо в одной локальной сети за межсетевым экраном, хотя, конечно, они могут работать и в открытой сети.

* *Распределение на основе сокетов*: используя TCP/IP сокеты, можно писать распределённые приложения, которые работают в *небезопасной* среде. Программная модель менее мощная, по сравнению с распределённым Эрлангом, но более безопасная. В разделе 10.5 *Распределение на основе сокетов*, мы увидим, как создавать приложения, используя простой распределённый механизм на основе сокетов.

Если вы восстановите в памяти материалы предыдущих глав, то вспомните, что основным элеменом программ является процесс. Создавать распределённые программы на Эрланге сравнительно легко: всё, что нам надо — это порождать наши процессы на рабочих компьютерах. Всё будет работать как и прежде.

Мы все уже привыкли к последовательному программированию. Написание распределённых программ обычно гораздо труднее. В этой главе мы рассмотрим ряд техник для написания простых распределённых программ. Несмотря на их простому, они могут эффективно решать поставленные задачи.

А начнём мы с ряда небольших примеров. Для них нам понадобится изучить только две вещи, а затем мы сможем создать нашу первую распределённую программу. Мы узнаем, как запускать узел Эрланга и как выполнять удалённый вызов процедуры на удалённом узле Эрланга.

Когда я разрабатываю распределённое приложение, я всегда работаю над программой в определённом порядке:

1. Я пишу и тестирую программу в обычной, нераспределённой сессии Эрланга. Это то, где мы были до текущего момента, так что здесь не возникнет никаких новых проблем.

2. Я тестирую программу на двух различных узлах Эрланга, работающих *на одном компьютере*.

3. Я тестирую программу на двух различных узлах Эрланга, работающих на *двух физически разделённых компьютерах*, находящихся либо в одной локальной сети, либо где-то в Интернете.
...
* Шаг №1: Написание и тестирование сервера имен в обычной нераспределенной Эрланг-системе.
* Шаг №2: Тестирование сервер имён на двух узлах на одной машине.
* Шаг №3: Тестирование сервер имён на двух различных узлах на двух различных машинах в одной локальной сети.
* Шаг №4: Тестирование сервер имён на двух различных узлах на двух различных машинах двух различных доменах в двух различных странах.
...
<dl>
  <dt><code>@spec kvs:start() -> true</code></dt>
  <dd>Запускает сервер; создаёт сервер с зарегистрированным именем <code>kvs</code>.</dd>	
  <dt><code>@spec kvs:store(Key, Value) -> true</code></dt>
  <dd>Связывает <code>ключ</code> и <code>значение</code>.</dd>	  
  <dt><code>@spec kvs:lookup(Key) -> {ok, Value} | undefined</code></dt>
  <dd>Ищет значения для <code>ключа</code> и возвращает <code>{ok, Value}</code>, если с ключом связано значение; в противном случае возвращает <code>undefined</code>.</dd>	
</dl>  
...
*Первое*: запускаем терминальную оболочку (Информация для пользователей ОС Windows: ознакомьтесь с Приложением B. После того, как вы получите доступ к окну оболочки, должна работать команда erl -name Node. Не забудьте указать корректные пути, чтобы вы могли найти файл `erl.exe` (имя должно быть примерно таким: `C:\Program Files\erl5.4.4\bin\erl.exe`).) и в ней запускаем распределённый узел Эрланга с именем *gandalf*. Затем запускаем сервер:
```erlang
	$ erl -sname gandalf
	(gandalf@localhost) 1> kvs:start().
	true
```
*Замечание для пользователей ОС Windows*: в системе Windows имя может оказаться не *localhost*. Если оно не *localhost*, то вам придётся использовать имя, которое Windows вернёт вместо *localhost*, во всех последующих командах.

Аргумент *-sname gandalf* означает «запустить узел Эрланга с именем *gandalf* на локальной машине». Заметьте, как Эрланг-оболочка пишет имя узла (Имя узла имеет вид *Name@Host*. Name и Host — это атомы и если они содержат какие-либо не атомные символы, то такие атомы должны быть в одинарных кавычках.) перед командной подсказкой.
...
Возможно, ваши команды выглядят иначе, но мы только что выполнили наше первое распределённое вычисление! Сервер работал на первом узле, а клиент — на втором.
