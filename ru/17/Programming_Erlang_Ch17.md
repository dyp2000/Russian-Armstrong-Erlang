...
Для того, чтобы её можно было легко вызывать из оболочки erl, мы заключили этот код в функцию
...
deadlock-у (?), он немедленно отменяет транзакцию и откатывает
все изменения, которые в пределах этой транзакции были сделаны.
...
это не понравилось, он сказал: "Нельзя называть её Amnesia
- мы не можем выпускать базу данных, которая, судя по названию,
...
Задаёт имя создаваемой таблицы (atom). По соглашению, это также имя соотвествующей Эрланг-записи (record) - каждая строка в таблице будет являться экземпляром этой записи.
...
{disc_copies, NodeList}
NodeList задаёт список узлов Эрланг, на которых будут храниться дисковые копии создаваемой таблицы. Когда мы используем эту опцию, система также создаёт RAM-копию таблицы на том узле, на котором было выполнено создание таблицы.
...
Когда таблица реплицируется на несколько узлов Эрланг, она синхронизируется, пока это возможно.
...
Вот так выглядят команды, создающие базу данных Mnesia database.
...
на всех узлах из списка NodeList (он должен содержать список реально сушествующих Эрланг-узлов).
