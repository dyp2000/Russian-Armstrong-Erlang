...
namespace :epub do
	...
	INDEX_FILEPATH = File.join(TMP_DIR, 'erlangbook.html')
	TARGET_FILEPATH = "erlangbook-#{$lang}.epub"
	...
	file INDEX_FILEPATH => HTML_FILES do
		index_file = File.open(INDEX_FILEPATH, 'w') do |file|
			...
			file << '<head>'
			file << '<title>Programming Erlang: Software for a Concurrent World</title>'
			file << '</head>'
			file << '<body>'
			file << "\n"
